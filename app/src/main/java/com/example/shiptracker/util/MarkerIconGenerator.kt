...
object MarkerIconGenerator {
    ...
    fun getTintedShipIcon(context: Context, ship: ShipState, colorInt: Int): Drawable {
        ...
        val cacheKey = if (isCluster) {
            ...
            "ship_${colorInt}_${isMoored}_${ship.isGhost}".hashCode()
        }
        ...
        val bitmap = bitmapCache.getOrPut(cacheKey) {
            ...
            if (isMoored) {
                ...
                // 🚨 If it's a ghost, make it semi-transparent!
                if (ship.isGhost) {
                    vectorDrawable.alpha = 130 // Roughly 50% opacity
                } else {
                    vectorDrawable.alpha = 255 // Solid
                }
    ...
    fun inferShipTypeFromName(shipName: String): Int {
        ...
        return when {
            // 🚨 NEW: Catch RNLI and Lifeboats and assign them Type 51 (Search and Rescue)
            upperName.contains("RNLI") || upperName.contains("LIFEBOAT") || 
            upperName.contains("RESCUE") || upperName.contains("SAR ") || 
            upperName.contains("COASTGUARD") -> 51
            ...
            upperName.contains("PILOT") || upperName.contains("PATROL") || 
            upperName.contains("SAFETY") || upperName.contains("WORKBOAT") || upperName.contains("DREDG") || 
            upperName.contains("RESEARCH") || upperName.contains("SURVEY") -> 31
    ...
    fun getShipAndroidColor(ship: ShipState): Int {
        // 🚨 NEW: Force bright warning red if flagged by Anomaly Detector
        if (ship.isAnomalyFlagged) {
            return Color.parseColor("#DC2626") // Crimson Red Warning
        }
